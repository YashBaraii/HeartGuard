...
class Input_Fields(BaseModel):
    ...
    chestPainType: str = Field(..., pattern="^(TA|ATA|NAP|ASY)$", description="Chest pain type")
    restingBP: float = Field(..., ge=80, le=200, description="Resting blood pressure in mmHg")
    cholesterol: float = Field(..., ge=0, le=1000, description="Cholesterol level in mg/dl")
    fastingBS: int = Field(..., ge=0, le=1, description="Fasting blood sugar > 120 mg/dl (1) or not (0)")
    restECG: str = Field(..., pattern="^(Normal|ST|LVH)$", description="Resting ECG results")
    maxHR: float = Field(..., ge=60, le=220, description="Maximum heart rate achieved")
    exerciseAngina: str = Field(..., pattern="^(Y|N)$", description="Exercise induced angina (Y or N)")
    oldpeak: float = Field(..., ge=0, le=10, description="ST depression induced by exercise")
    stSlope: str = Field(..., pattern="^(Up|Flat|Down)$", description="Slope of peak exercise ST segment")
...
try:
    ...
    expected_cols = pickle.load(open(os.path.join(BASE_DIR, "models", "columns.pkl"), "rb"))
